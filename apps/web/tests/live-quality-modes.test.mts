...
const cameraTilePath = new URL('../src/components/CameraTile.tsx', import.meta.url);
const recordingManagerPath = new URL('../../api/src/recordings/recording-process-manager.service.ts', import.meta.url);
...
test('ao vivo não tem botão superior de gravar e identifica a ação manual no tile', async () => {
  const [livePage, tile] = await Promise.all([
    readFile(livePagePath, 'utf8'),
    readFile(cameraTilePath, 'utf8'),
  ]);
  assert.doesNotMatch(livePage, /\{isRecording \? 'Gravando' : 'Gravar'\}/);
  assert.match(tile, /gravação manual \(máximo de 10 minutos\)/);
  assert.match(tile, /isManualRecordingActive \? 'Manual' : 'Gravar'/);
});

test('gravação manual é limitada a dez minutos pelo servidor', async () => {
  const source = await readFile(recordingManagerPath, 'utf8');
  assert.match(source, /async startManualRecording\(cameraId: string, segmentSeconds: number, maxDurationSeconds = 600\)/);
  assert.match(source, /Math\.min\(600, Math\.max\(10,/);
  assert.match(source, /void this\.stopManualRecording\(cameraId\)/);
});
